...
class Game_2048():
    ...
    def leftKey(self, dummy):
        if (self.horizontalMoveExists() or self.zeroExist()) and self.flag:
            self.stack()
            self.combine()
            self.stack()
            self.addNewTile()
    ...
    def upKey(self, dummy):
        if (self.verticalMoveExists() or self.zeroExist()) and self.flag:
            self.transpose()
            self.stack()
            self.combine()
            self.stack()
    ...
    def gameOver(self):
        if any([2048 in i for i in self.matrix]):
            self.flag = False
            self.text = "Winner!"
        elif (not self.zeroExist()) and (not self.horizontalMoveExists()) and (not self.verticalMoveExists()):
            self.flag = False
            self.text = "Game Over!"
    ...
    def replayGame(self):
        if self.flag == False:
            self.result_frame = Frame(self.main_grid, bg = co.MAIN_CANVAS)
            self.result_frame.place(relx = 0.1, rely = 0.3, relheight = 0.4, relwidth = 0.8)

            self.result = Label(self.result_frame, bg = co.MAIN_CANVAS, fg = 'black', font = co.GAME_OVER_FONT, text = self.text, highlightthickness = 3, highlightcolor = 'black')
            self.result.place(relx = 0, rely = 0, relheight = 0.45, relwidth = 1)

            self.replay_game_button = Button(self.result_frame, bg = "light gray", fg = 'black', font = ("helvetica", 20, 'bold'), text = "Play Again", command = self.replayGameLogic)
            self.replay_game_button.place(relx = 0, rely = 0.5, relheight = 0.45, relwidth = 0.5)

            self.end_game_button = Button(self.result_frame, bg = "light gray", fg = 'black', font = ("helvetica", 20, 'bold'), text = "Quit", command = self.endGame)
            self.end_game_button.place(relx = 0.5, rely = 0.5, relheight = 0.45, relwidth = 0.5)      

    def replayGameLogic(self):
        self.flag = True
